...
let getEduProgramAttributes = async eduProgramPageRoot => new Map([...await Promise.all(eduProgramPageRoot.findByClass("table-edu")[0].findByTag("tbody")[0].children.map(async attr => [attr.findByTag("th")[0].text, turndown(await minifyHtml(attr.findByTag("td")[0].innerHtml))])), ["EDU PROGRAM", eduProgramPageRoot.findByClass("edu-h1")[0].text], ["EDU PROFILE", eduProgramPageRoot.findByClass("edu-h2")[0].text]]);
